...
router.get('/admin-layout.js', (req, res) => {
  res.sendFile(path.join(__dirname, 'admin-layout.js'));
});
...
const pages = ['dashboard', 'bookings', 'walkins', 'services', 'availability', 'staff', 'payments', 'audit'];
...
router.get('/bookings/pickup-pending', async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT
        b.id, b.reference_code, b.guest_name, b.status,
        s.name AS service_name,
        p.is_fully_paid, p.remaining_balance
      FROM bookings b
      LEFT JOIN services  s ON s.id = b.service_id
...
router.post('/bookings/walkin', async (req, res) => {
  try {
    ...
    const variantResult = await pool.query(
      'SELECT price FROM service_variants WHERE id = $1', [variant_id]
    );
    ...
    const availResult = await pool.query(
      `SELECT id FROM availability
       WHERE service_id = $1 AND date = CURRENT_DATE AND is_open = true LIMIT 1`,
      [service_id]
    );
...
router.get('/services', async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT
        s.id, s.name, s.description, s.price, s.duration_hours, s.is_active,
        json_agg(
...
router.get('/availability', async (req, res) => {
  try {
    ...
    let query = `
      SELECT
        a.id, a.date, a.capacity, a.is_open,
        s.name AS service_name,
...
router.get('/payments', async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT
        p.id, p.amount, p.amount_paid, p.remaining_balance,
        p.payment_type, p.is_fully_paid, p.status, p.paid_at,
        b.reference_code, b.guest_name
      FROM payments p
...
router.get('/audit-logs', async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT
        al.id, al.action, al.target_table, al.target_id, al.details, al.created_at,
        u.name AS user_name
      FROM audit_logs al
